...
//		gs.clearStatistics();
    ...
    private void buildPanels(List<StatisticsOverlay.Statistic> statistics) {
        // Possession
        List<StatisticsOverlay.Statistic> possessionStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticType.POSSESSION))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftPossessionStatistics = possessionStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Offside
        List<StatisticsOverlay.Statistic> offsideStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.OFFSIDE))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftOffsideStatistics = offsideStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Foul
        List<StatisticsOverlay.Statistic> foulStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.FOUL))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftFoulStatistics = foulStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Corner
        List<StatisticsOverlay.Statistic> cornerStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.CORNER))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftCornerStatistics = cornerStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Free Kick
        List<StatisticsOverlay.Statistic> freeKickStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.FREE_KICK))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftFreeKickStatistics = freeKickStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Pass
        List<StatisticsOverlay.Statistic> passStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.PASS))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftPassStatistics = passStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Goal Kick
        List<StatisticsOverlay.Statistic> goalKickStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.GOAL_KICK))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftGoalKickStatistics = goalKickStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        // Goal Kick
        List<StatisticsOverlay.Statistic> kickInStatistics = statistics.stream()
                .filter(statistic -> statistic.type.equals(StatisticsOverlay.StatisticType.KICK_IN))
                .collect(Collectors.toList());

        List<StatisticsOverlay.Statistic> leftKickInStatistics = kickInStatistics.stream()
                .filter(statistic -> statistic.team == 1)
                .collect(Collectors.toList());

        ArrayList<String> titlesOne = new ArrayList<>(Arrays.asList("Fouls", "Offsides", "Corners", "Free kicks", "Goal Kicks", "Kick ins"));
        ArrayList<List<String>> valuesOne = new ArrayList<>();

        valuesOne.add(Arrays.asList(
                String.valueOf(leftFoulStatistics.size()),
                String.valueOf(foulStatistics.size() - leftFoulStatistics.size())
        ));
        valuesOne.add(Arrays.asList(
                String.valueOf(leftOffsideStatistics.size()),
                String.valueOf(offsideStatistics.size() - leftOffsideStatistics.size())
        ));
        valuesOne.add(Arrays.asList(
                String.valueOf(leftCornerStatistics.size()),
                String.valueOf(cornerStatistics.size() - leftCornerStatistics.size())
        ));
        valuesOne.add(Arrays.asList(
                String.valueOf(leftFreeKickStatistics.size()),
                String.valueOf(freeKickStatistics.size() - leftFreeKickStatistics.size())
        ));
        valuesOne.add(Arrays.asList(
                String.valueOf(leftGoalKickStatistics.size()),
                String.valueOf(goalKickStatistics.size() - leftGoalKickStatistics.size())
        ));
        valuesOne.add(Arrays.asList(
                String.valueOf(leftKickInStatistics.size()),
        ...
        if (possessionStatistics.size() > 0) {
            ...
            valuesOne.add(Arrays.asList(
                    (Math.round((leftPossessionStatistics.size() / totalSize) * 100) + "%"),
    ...
    void drawStatistics(GL2 gl, int x, int y, int w, int h, int screenW, int screenH, float opacity) {
        ...
        for (int i = 0; i < panel.getTitles().size(); i++) {
            tr2.draw(panel.getTitles().get(i), x + w / 2 - (int) tr2.getBounds(panel.getTitles().get(i)).getWidth() / 2, LINE_Y);
            tr2.draw(panel.getValues().get(i).get(0), x + Y_PAD, LINE_Y);
            tr2.draw(panel.getValues().get(i).get(1), x + w - Y_PAD - (int) tr2.getBounds(panel.getValues().get(i).get(1)).getWidth(), LINE_Y);
            LINE_Y -= (LINE_HEIGHT);
        }
        tr2.endRendering();
    }
    ...
    public static boolean shouldDisplayStatistics(long currentTimeMillis, GameState gs) {
        if (!gs.isInitialized() || !gs.isPlaying()) return false;
        return true;
//		float dt = (currentTimeMillis - DEPLOY_TIME) / 1000.0f;
...
//		return dt < PANEL_SHOW_TIME + PANEL_FADE_TIME;
                    ...
                    case GameState.PLAY_MODE:
                        ...
                        switch (playMode) {
                            ...
                            case GameState.PASS_LEFT:
                                statistic.type = StatisticsOverlay.StatisticType.PASS;
                                statistic.team = 1;
                                statistic.index = Integer.parseInt(atoms[1]);
    ...
    private void calculateStateStatistics(GameState gs) {
        ...
        calculatePossession(gs, world);
    }

    private void calculatePossession(GameState gs, WorldModel world) {
        ...
        boolean leftTeamPossession = true;
        int agentId = -1;

        try {
            for (Agent agent : leftTeam.getAgents()) {
                float distanceToBall = Math.abs(agent.getPosition().lengthSquared() - ball.getPosition().lengthSquared());
            ...
            addStatistic(new Statistic(System.currentTimeMillis(), StatisticType.POSSESSION, leftTeamPossession ? 1 : 2, agentId));
        } catch (NullPointerException e) {
            System.err.println("Still initializing statistics");
        }
    }
